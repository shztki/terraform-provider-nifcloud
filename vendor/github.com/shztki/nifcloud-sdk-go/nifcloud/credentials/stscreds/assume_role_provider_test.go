...
func (s *stubSTS) AssumeRole(input *sts.AssumeRoleInput) (*sts.AssumeRoleOutput, error) {
	...
	return &sts.AssumeRoleOutput{
		Credentials: &sts.Credentials{
			...
			SecretAccessKey: nifcloud.String("assumedSecretAccessKey"),
			SessionToken:    nifcloud.String("assumedSessionToken"),
			Expiration:      &expiry,
		},
	}, nil
}
...
func TestAssumeRoleProvider_WithTokenCode(t *testing.T) {
	...
	p := &AssumeRoleProvider{
		Client:       stub,
		RoleARN:      "roleARN",
		SerialNumber: nifcloud.String("0123456789"),
		TokenCode:    nifcloud.String("code"),
	}
