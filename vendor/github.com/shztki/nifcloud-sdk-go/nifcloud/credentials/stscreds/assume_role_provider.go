...
credentials.
	...
	creds := stscreds.NewCredentials(sess, "myRoleArn", func(p *stscreds.AssumeRoleProvider) {
		p.SerialNumber = nifcloud.String("myTokenSerialNumber")
		p.TokenCode = nifcloud.String("00000000")
	})
...
func (p *AssumeRoleProvider) Retrieve() (credentials.Value, error) {
	...
	input := &sts.AssumeRoleInput{
		DurationSeconds: nifcloud.Int64(int64(p.Duration / time.Second)),
		RoleArn:         nifcloud.String(p.RoleARN),
		RoleSessionName: nifcloud.String(p.RoleSessionName),
	...
	if p.SerialNumber != nil {
		if p.TokenCode != nil {
			...
			input.TokenCode = nifcloud.String(code)
		} else {
