...
func TestNewDefaultSession(t *testing.T) {
	...
	s := New(&nifcloud.Config{Region: nifcloud.String("region")})
	...
	assert.Equal(t, nifcloud.LogOff, *s.Config.LogLevel)
}
...
func TestNew_WithCustomCreds(t *testing.T) {
	...
	s := New(&nifcloud.Config{Credentials: customCreds})
...
func TestNew_WithSessionLoadError(t *testing.T) {
	...
	logger := bytes.Buffer{}
	s := New(&nifcloud.Config{Logger: &mockLogger{&logger}})
...
func TestSessionCopy(t *testing.T) {
	...
	newSess := s.Copy(&nifcloud.Config{Region: nifcloud.String("new_region")})
...
func TestSessionClientConfig(t *testing.T) {
	s, err := NewSession(&nifcloud.Config{
		Credentials: credentials.AnonymousCredentials,
		Region:      nifcloud.String("orig_region"),
	...
	cfg := s.ClientConfig("mock-service", &nifcloud.Config{Region: nifcloud.String("other-region")})
...
func TestSesisonAssumeRole(t *testing.T) {
	...
	s, err := NewSession(&nifcloud.Config{Endpoint: nifcloud.String(server.URL), DisableSSL: nifcloud.Bool(true)})
...
func TestSessionAssumeRole_WithMFA(t *testing.T) {
	...
	sess, err := NewSessionWithOptions(Options{
		...
		Config: nifcloud.Config{
			Region:     nifcloud.String("us-east-1"),
