...
func TestLogRequest(t *testing.T) {
	cases := []struct {
		Body       io.ReadSeeker
		ExpectBody []byte
		LogLevel   nifcloud.LogLevelType
	}{
		{
			Body:       nifcloud.ReadSeekCloser(bytes.NewBuffer([]byte("body content"))),
			ExpectBody: []byte("body content"),
		},
		{
			Body:       nifcloud.ReadSeekCloser(bytes.NewBuffer([]byte("body content"))),
			LogLevel:   nifcloud.LogDebugWithHTTPBody,
			ExpectBody: []byte("body content"),
		},
		{
	...
	for i, c := range cases {
		...
		req := request.New(
			nifcloud.Config{
				...
				LogLevel:    nifcloud.LogLevel(c.LogLevel),
			},
			metadata.ClientInfo{
