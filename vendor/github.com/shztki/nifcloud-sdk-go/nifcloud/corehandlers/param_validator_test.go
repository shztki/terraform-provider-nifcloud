...
func TestNoErrors(t *testing.T) {
	input := &StructShape{
		...
		RequiredMap: map[string]*ConditionalStructShape{
			"key1": {Name: nifcloud.String("Name")},
			"key2": {Name: nifcloud.String("Name")},
		},
		RequiredBool:   nifcloud.Bool(true),
		OptionalStruct: &ConditionalStructShape{Name: nifcloud.String("Name")},
	}
...
var testsFieldMin = []struct {
...
}{
	{
		...
		in: testInput{StringField: nifcloud.String("abcd")},
	},
	{
		err: func() awserr.Error {
			invalidParams := request.ErrInvalidParams{Context: "testInput"}
		...
		in: testInput{StringField: nifcloud.String("abcd"), ListField: []string{"a", "b"}},
	},
	{
		err: func() awserr.Error {
			invalidParams := request.ErrInvalidParams{Context: "testInput"}
		...
		in: testInput{StringField: nifcloud.String("abcd"), ListField: []string{"a", "b"}, MapField: map[string]string{"a": "a", "b": "b"}},
	},
	{
		err: nil,
		in: testInput{StringField: nifcloud.String("abcde"),
...
func BenchmarkValidateAny(b *testing.B) {
	input := &kinesis.PutRecordsInput{
		StreamName: nifcloud.String("stream"),
	}
	for i := 0; i < 100; i++ {
		record := &kinesis.PutRecordsRequestEntry{
			Data:         make([]byte, 10000),
			PartitionKey: nifcloud.String("partition"),
		}
