...
func TestValidateEndpointHandler(t *testing.T) {
	...
	svc := awstesting.NewClient(nifcloud.NewConfig().WithRegion("us-west-2"))
	svc.Handlers.Clear()
...
func TestValidateEndpointHandlerErrorRegion(t *testing.T) {
	...
	if e, a := nifcloud.ErrMissingRegion, err; e != a {
...
func TestSendWithContextCanceled(t *testing.T) {
	c := awstesting.NewClient(&nifcloud.Config{
...
func TestValidateReqSigHandler(t *testing.T) {
	cases := []struct {
		...
		{
			Req: &request.Request{
				Config: nifcloud.Config{Credentials: credentials.AnonymousCredentials},
...
func TestBuildContentLength_ZeroBody(t *testing.T) {
	...
	svc := s3.New(unit.Session, &nifcloud.Config{
		Endpoint:         nifcloud.String(server.URL),
