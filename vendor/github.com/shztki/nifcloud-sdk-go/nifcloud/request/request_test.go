...
func TestRequestRecoverRetry5xx(t *testing.T) {
	...
	s := awstesting.NewClient(nifcloud.NewConfig().WithMaxRetries(10))
...
func TestRequestExhaustRetries(t *testing.T) {
	...
	s := awstesting.NewClient(nifcloud.NewConfig().WithSleepDelay(sleepDelay))
	s.Handlers.Validate.Clear()
	s.Handlers.Unmarshal.PushBack(unmarshal)
	s.Handlers.UnmarshalError.PushBack(unmarshalError)
	s.Handlers.Send.Clear() // mock sending
...
func TestRequestRecoverExpiredCreds(t *testing.T) {
	...
	s := awstesting.NewClient(&nifcloud.Config{MaxRetries: nifcloud.Int(10), Credentials: credentials.NewStaticCredentials("AKID", "SECRET", "")})
	s.Handlers.Validate.Clear()
	s.Handlers.Unmarshal.PushBack(unmarshal)
	s.Handlers.UnmarshalError.PushBack(unmarshalError)
...
func TestRequestUserAgent(t *testing.T) {
	s := awstesting.NewClient(&nifcloud.Config{Region: nifcloud.String("us-east-1")})
	//	s.Handlers.Validate.Clear()
	...
	expectUA := fmt.Sprintf("foo/bar %s/%s (%s; %s; %s)",
		nifcloud.SDKName, nifcloud.SDKVersion, runtime.Version(), runtime.GOOS, runtime.GOARCH)
...
func TestRequestRecoverTimeoutWithNilBody(t *testing.T) {
	...
	s.Handlers.AfterRetry.PushBack(func(r *request.Request) {
		if r.Error != nil {
			r.Error = nil
			r.Retryable = nifcloud.Bool(true)
			r.RetryCount++
		}
	})
	s.Handlers.Send.Clear() // mock sending
...
func TestRequest_NoBody(t *testing.T) {
	...
	for i, c := range cases {
		...
		s := awstesting.NewClient(&nifcloud.Config{
			Region:     nifcloud.String("mock-region"),
			MaxRetries: nifcloud.Int(0),
		...
		in := struct {
			...
			Bucket: nifcloud.String("mybucket"), Key: nifcloud.String("myKey"),
		}
...
func TestWithLogLevel(t *testing.T) {
	...
	opt := request.WithLogLevel(nifcloud.LogDebugWithHTTPBody)
	r.ApplyOptions(opt)

	if !r.Config.LogLevel.Matches(nifcloud.LogDebugWithHTTPBody) {
...
func TestEnforceShouldRetryCheck(t *testing.T) {
	...
	s := awstesting.NewClient(&nifcloud.Config{
		...
		Retryer:                 retryer,
		HTTPClient:              client,
		EnforceShouldRetryCheck: nifcloud.Bool(true),
	})
...
func TestRequestWillRetry_ByBody(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			WillRetry:  true,
			HTTPMethod: "POST",
			Body:       nifcloud.ReadSeekCloser(bytes.NewReader([]byte("abc123"))),
		},
		{
			WillRetry:   true,
			HTTPMethod:  "GET",
		...
		},
		{
			WillRetry:  false,
			HTTPMethod: "POST",
			Body:       nifcloud.ReadSeekCloser(bytes.NewBuffer([]byte("abc123"))),
		},
	}
	...
	for i, c := range cases {
		...
		req.Retryable = nifcloud.Bool(true)
