...
func TestWaiterPathAll(t *testing.T) {
	svc := &mockClient{Client: awstesting.NewClient(&nifcloud.Config{
	...
	resps := []*MockOutput{
		{ // Request 1
			States: []*MockState{
				{State: nifcloud.String("pending")},
				{State: nifcloud.String("pending")},
			},
		},
		{ // Request 2
			States: []*MockState{
				{State: nifcloud.String("running")},
				{State: nifcloud.String("pending")},
			},
		},
		{ // Request 3
	...
	w := request.Waiter{
		...
		SleepWithContext: nifcloud.SleepWithContext,
	...
	err := w.WaitWithContext(nifcloud.BackgroundContext())
...
func TestWaiterFailure(t *testing.T) {
	...
	resps := []*MockOutput{
		...
		{ // Request 3
			States: []*MockState{
				{State: nifcloud.String("running")},
				{State: nifcloud.String("stopping")},
			},
		},
	}
	...
	err := w.WaitWithContext(nifcloud.BackgroundContext()).(awserr.Error)
	assert.Error(t, err)
...
func TestWaiter_ApplyOptions(t *testing.T) {
	...
	logger := nifcloud.NewDefaultLogger()

	w.ApplyOptions(
		request.WithWaiterLogger(logger),
		request.WithWaiterRequestOptions(request.WithLogLevel(nifcloud.LogDebug)),
	...
	if e, a := nifcloud.LogDebug, r.Config.LogLevel.Value(); e != a {
...
func TestWaiter_WithContextCanceled(t *testing.T) {
	...
	w.SleepWithContext = func(c nifcloud.Context, delay time.Duration) error {
