...
type Signer struct {
	...
	// nifcloud.LogLevelType for more information on available logging levels
	//
...
func (v4 Signer) signWithBody(r *http.Request, body io.ReadSeeker, service, region string, exp time.Duration, isPresign bool, signTime time.Time) (http.Header, error) {
	...
	if v4.Debug.Matches(nifcloud.LogDebugWithSigning) {
...
func signSDKRequestWithCurrTime(req *request.Request, curTimeFn func() time.Time, opts ...func(*Signer)) {
	...
	if region == "" {
		region = nifcloud.StringValue(req.Config.Region)
	}
...
func (ctx *signingCtx) buildBodyDigest() error {
	...
	if hash == "" {
		if ctx.unsignedPayload || (ctx.isPresign && ctx.ServiceName == "s3") {
			...
			if !nifcloud.IsReaderSeekable(ctx.Body) {
