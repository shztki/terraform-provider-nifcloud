resource "nifcloud_instance" "example_server_cent_noneglobal" {
  ...
  name     = "${format("%s%03d", "${lookup(var.instance_cent, "name")}", count.index + 101)}"
  ...
  description       = "${format("%s%03d", "${lookup(var.instance_cent, "memo")}", count.index + 101)}"
  user_data         = "${file(format("%s_%03d.sh", "${lookup(var.instance_cent, "user_data")}", count.index + 101))}"
